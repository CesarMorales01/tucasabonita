...
<div style="display: flex; align-items: center; justify-content: center; width: 650px; margin: 0 auto;"  class="container">
  <div class="row ">
    <nav style="background-color:<?php echo $get_settings0['btituloTres']; ?>"  class="navbar navbar-expand-md">
      ...
      <div class="col-xl-3">
        <a class="navbar-brand" style="color:black; padding-left:30px; padding-right:30px;" href="Form_buscar_clientes_web.php"> <i class="fas fa-search"></i>  Buscar clientes</a>
      </div>
      <div class="col-xl-3">
...
// inicio div titulo cuadrando cuentas
echo '<div style="width: 650px; text-align:center; margin: 0 auto;" class="container">';
echo '<form method="post" action="Form_Cuadrar_cuentas_cobrox.php">';   
echo '<select name="Cobro">';    
$mysql=new mysqli($hostname_localhost,$username_localhost,$password_localhost,$database_localhost);
if ($mysql->connect_error) die("Problemas con la conexión a la base de datos");
// no creer sser necesario....
$registros2=$mysql->query("select * from asesores where imei=$revisar_sesion_comis") or die ("problemas en la consulta1");
while($reg2=$registros2->fetch_array()){
$asesorx=$reg2['nombre'];
$imeix=$reg2['imei'];
}
// SELECCIONAR SOLO CASA BONITA
$casabonita="Casabonita";
$casabonita="'".$casabonita."'";
$registros1=$mysql->query("select * from Carteras where Nombre=$casabonita") or die ("problemas en la consulta1");	  
...
echo '</form>';
$row_cnt = $registros1->num_rows;
if($row_cnt<=0){
	$Cobro0=" ";
	echo '<h1>No se han encontrado carteras registradas.</h1>';
	echo '<a href="Form_carteras.php">Puedes ir a "Administrar carteras" donde podrás crear una.</a>';
    echo '<br>';
	echo '<br>';
	}	
/*	
//ESTABLECER CARTERA PREDETERMINADA
echo '<form method="post" action="Form_cartera_prede.php">';
$xcomparar=$Cobro0;
...
$Cobro0=$get_prede['variable'];
    if($xcomparar==$Cobro0){
     echo '<input type="hidden" value='.$Cobro0.' name="cartera">';
     echo '<input type="hidden" value='.$revisar_sesion.' name="asesor">';
     echo '<input  class="botonsubmit" type="submit" value="'.$Cobro0.' establecida como cartera predeterminada">';  
    } else {      
 ...
 echo '<input type="hidden" value='.$Cobro0.' name="cartera">';
 echo '<input type="hidden" value='.$revisar_sesion.' name="asesor">';
 echo '<input  class="botonsubmit" type="submit" value="Establecer '.$Cobro0.' como cartera predeterminada">';  
}   
echo '</form>'; 
*/
echo '<br>';
echo '<h1>Cuadrando cuentas '.$Cobro0.'</h1>';
...
// inicio div tabla ingresos y egresos 
...
$consultar_total_prestado=$mysql->query("SELECT sum(valorprestamo) as total_prestado_dia FROM creditos_casa_bonita where fecha_prest=$get_global_fecha_hoy") or die ("problemas en consultar total prestado");
   ...
   $get_total_cobrado=$consultar_total_prestado->fetch_array();	  
	...
	// 2 FILA
      ...
      echo '</td>'; 
		echo '<td>';	  
		$c=$mysql->query("SELECT * FROM lista_compras where fecha like '%$get_global_fecha_hoy_comis%'") or die ("problemas en consultar total contado");
		$sumar_contados=0;
		while($g=$c->fetch_array()){
			if ($g['periodicidad']=="No aplica"){
				$sumar_contados=$sumar_contados+$g['total_compra'];
			}
		}
		echo '<a href="Lista_prestado_hoy.php?Cobro='.$Cobro.'">';
     ...
     echo '<a href="Form_Lista_prestado_select_dia.php?fecha='.$get_global_fecha_hoy_comis.'&Cobro='.$Cobro.'$asesor='.$asesorx.'">Mostrar creditos y ventas seleccionando dia</a>';
     ...
     echo '<a href="Cuadrar_caja.php?Cobro='.$Cobro0.'">Caja</a>';
      ...
      echo '<a href="Form_cobrado_select_rango.php?Cobro='.$Cobro.'">Total cobrado seleccionando rango</a>';
     echo '</td>';  
      echo '<td>';

     echo '<a href="Form_prestado_select_rango.php?Cobro='.$Cobro.'">Total ventas seleccionando rango</a>';
...
echo '<div class="col-4">';
      // TABLA OPCIONES 
echo '<table class="tabencabezado" style="margin: 0 auto;">';
echo '<tr><th colspan="2" style="text-align:center">Otras Opciones</th> </tr>';	
...
//1 FILA
	...
	echo '<tr>';
      ...
      echo '<a href="Form_lista_cobrado.php?cobro='.$Cobro0.'">Lista cobrandoApp</a>';
     ...
     echo '</tr>'; 
      ...
      echo '<td>';
	   ...
	   echo '<a href="Advertencias.php?cobro='.$Cobro0.'">Varios</a>';
      ...
      echo '<tr>';
      echo '<td>';
      echo '<a href="Cuentas_canceladas_hoy.php?Cobro='.$Cobro.'">Cuentas canceladas hoy</a>';
      echo '</td>'; 
       echo '</tr>';
       echo '<tr>';
       echo '<td>';
       echo '<a href="Ver_lista_revisados.php?Cobro='.$Cobro0.'">Ver lista revisados</a>';
       echo '</td>'; 
        echo '</tr>';  
     echo '</table>';
...
echo '<div style="display: flex; align-items: center; justify-content: center; width: 650px; margin: 0 auto;"  id="divTotales">'; 
     ...
     echo '<tr><th colspan="6"><a href="OpcionesTotales.php?Cobro='.$Cobro.'">TOTALES</a></th>';
     echo '<th colspan="2">  <a href="Guardar_totales_actual.php?Cobro='.$Cobro.'">Guardar totales actual</a> </th>';
     echo '</tr>';	
     echo '<tr><th>PERIODO</th> <th>TOTAL CUENTAS</th> <th>TOTAL ABONOS</th><th>%</th> <th>TOTAL SALDOS</th><th>%</th><th> <a href="Fechax_clientes_enmora.php?Cobro='.$Cobro.'&fecha='.$get_global_fecha_hoy_comis.'">TOTAL CARTERA EN MORA</a></th><th>%</th></tr>';
//1 FILA
	$consultar_totales=$mysql->query("SELECT sum(totalapagar), sum(tt_abonos), sum(tt_saldo)  FROM creditos_casa_bonita where Cobro=$Cobro") or die ("problemas en consultar totales");
...
$consultar_cartera_enmora=$mysql->query("SELECT  sum(tt_saldo)  FROM `creditos_casa_bonita` where vencimiento < $get_global_fecha_hoy and Cobro=$Cobro") or die("problemas al consultar cartera en mora");      
...
//CONSULTANDO NUMERO DE CLIENTES
    ...
    $get_nclientes=$consultar_nclientes->fetch_array();	
  	...
  	$consultar_nclientes_prest=$mysql->query("select count(*) from creditos_casa_bonita where Cobro=$Cobro") or die ("problemas en consultar nclientes");
...
echo '<br><br>';
echo '<tr><th>PERIODO</th><th style="text-align:center;" ><a href="Form_aprobar_credito.php?Cobro='.$Cobro.'">APROBAR CREDITO</a></th><th style="text-align:center;"><a href="Clientes_con_prestamo.php?Cobro='.$Cobro.'">TOTAL PRESTAMOS</a></th><th style="text-align:center;"><a href="Clientes_sin_saldo.php?Cobro='.$Cobro.'">CLIENTES SIN SALDO</a></th></tr>';
echo '<tr>';
