...
$registros=$mysql->query("SELECT nombre, clientes.cedula, fecha, valor_abono from abonos_creditos_casa_bonita JOIN clientes on abonos_creditos_casa_bonita.cedula=clientes.cedula where asesor=$asesor and ($fecha_inicial <= fecha AND fecha <= $fecha_final) ORDER BY fecha");

$registros1=$mysql->query("SELECT sum(valor_abono) from abonos_creditos_casa_bonita where asesor=$asesor and($fecha_inicial <= fecha AND fecha <= $fecha_final)")  or die ("problemas en la consulta sumatorissss");
...
if($fechas_duplicadas!=null){
	echo "<a href='cobrado_rango_duplicados_micro.php?fechas_duplicadas=".serialize($fechas_duplicadas)."'>Revisar registros duplicados!!!</a>"; 
}	
