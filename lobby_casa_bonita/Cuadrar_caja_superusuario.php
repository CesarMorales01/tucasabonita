 ...
 echo '<table class="tablalistado1"style="margin: 0 auto;">';
    ...
    while ($reg=$registros->fetch_array()) {
	  ...
	  if($get_fecha==$reg['fecha']){
	   echo '<br>';
	   echo '<a href="alerta_eliminar_caja_superusuario.php?id='.$reg['id'].'&Cobro='.$Cobro1.'" >Eliminar registro</a>';  
	 } 
      echo '</td>';
      echo '</tr>';      
...
$borrar_uid=$mysql->query("delete from caja where id=$ufecha") or die ("Problemas borrar primera fila");
 ...
 echo '<br>';
echo '<form method="post" action="nuevo_ingreso_superusuario.php">';
...
echo '<br>';
echo '<br>';
echo '<a href="Form_Cuadrar_cuentas_superusuario.php">Ir a cuadrar cuentas</a>';
